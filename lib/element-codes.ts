...
const PT_CATEGORY_NUMBERS: Record<string, number> = {
  ...
  "g. electrogeno": 8,
  sais: 9,
  "l. vida y anclajes": 10,
  bms: 11,
  "puertas aut.": 12,
  ite: 13,
  "inst. petroliferas": 14,
  "carg. electricos": 15,
  megafonia: 16,
  "z. infantiles": 17,
  "inst. fotovoltaica": 18,
  ddd: 19,
  "calidad aire": 20,
  "medio ambiente": 21,
  licencias: 22,
...
function hierarchyKey(
  ...
  return [category, installation, action]
    .map(normalizeLabel)
    .join("|");
}
 ...
 *
 * 1º nivel = Descripción / categoría
 * 2º nivel = Instalación
 * 3º nivel = Actuación
 */
export function buildElementCodes<T extends ElementCodeItem>(
  catalog: T[],
) {
  ...
  return catalog.map(item => {
    ...
    const categoryKey =
      `${normalizeLabel(item.country)}|${normalizeLabel(item.category)}`;

    const installationKey =
      `${categoryKey}|${normalizeLabel(item.installation)}`;

    if (!installationNumbers.has(installationKey)) {
      const installationOrdinal =
        countKeysWithPrefix(
          installationNumbers,
          `${categoryKey}|`,
        ) + 1;

      installationNumbers.set(
        installationKey,
        installationOrdinal,
      );
    }
    ...
    const installationOrdinal =
      installationNumbers.get(installationKey) || 1;

    const baseCode =
      `${categoryNumber}.${installationOrdinal}`;

    const actionKey =
      `${installationKey}|${normalizeLabel(item.action)}`;
    ...
    const actionOrdinal =
      actionNumbers.get(actionKey) || 1;

    const actionCode =
      `${baseCode}.${actionOrdinal}`;
...
/**
 * Añade el cuarto dígito únicamente cuando existen
 * varias unidades de la misma combinación:
 *
 * Descripción + Instalación + Actuación.
 *
 * Ejemplo:
 *
 * 1.1.1
...
export function addDisplayCodes<T extends ElementCodeItem>(
  ...
  const catalogWithCodes =
    buildElementCodes(catalog);
  ...
  resolved.forEach(item => {
    ...
    counts.set(
      item.actionCode,
      (counts.get(item.actionCode) || 0) + 1,
    );
  });
