...
export function AppShell({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    ...
    setRole(state.role);
    setCountry(state.country || "España");
  }, []);
  ...
  function changeCountry(v: "España" | "Portugal") {
  ...
  return (
    <div className="min-h-screen">
      <aside
        className={`fixed inset-y-0 left-0 z-40 bg-[#002A54] text-white transition-all duration-300 ${
      ...
      >
          ...
          >
            ...
            {!sidebarCollapsed && (
              <div>
                ...
                <div className="text-[10px] uppercase tracking-[.2em] text-white/60">
                  Technical Legal
                </div>
              </div>
            )}
        ...
        <div className="p-4">
          ...
          <nav className="space-y-1">
            {nav
              .filter((n) => n.href !== "/settings" || role === "ADMIN")
              .map((n) => {
                const I = n.icon;
                ...
                return (
                  <Link
                    ...
                    className={`flex items-center gap-3 rounded-xl px-3 py-2.5 text-sm transition ${
                      active
                        ? "bg-[#FFCC00] font-bold text-[#002A54]"
        ...
        <header className="sticky top-0 z-20 flex min-h-16 items-center gap-3 border-b border-slate-200 bg-white/95 px-4 py-2 backdrop-blur lg:px-8">
          ...
          <div className="ml-auto flex items-center gap-2">
            ...
            <select
              value={role}
              onChange={(e) =>
                changeRole(e.target.value as V1Role)
              }
            ...
            {role === "ADMIN" ? (
              <select
                ...
                onChange={(e) =>
                  changeCountry(
                    e.target.value as "España" | "Portugal"
                  )
                }
              ...
              <div className="hidden rounded-xl border border-slate-200 px-3 py-2 text-xs font-semibold md:block">
                {country === "España"
                  ? "ES · España"
                  : "PT · Portugal"}
              </div>
            ...
            <div className="h-9 w-9 rounded-full bg-slate-900 text-center text-xs font-bold leading-9 text-white">
              {role === "ADMIN"
                ? "AD"
                : role === "GESTOR"
                ? "GE"
