...
export function AppShell({children}:{children:React.ReactNode}) {
 ...
 return <div className="min-h-screen">
  <aside className={`fixed inset-y-0 left-0 z-40 bg-[#002A54] text-white transition-all duration-300 ${open?"translate-x-0":"-translate-x-full lg:translate-x-0"} ${sidebarCollapsed?"lg:w-20":"w-72"}`}>
  <div className="flex h-20 items-center justify-between border-b border-white/10 px-4">
    <Link href="/" onClick={()=>setOpen(false)} className="flex items-center gap-3 overflow-hidden">
      <div className="flex h-11 w-11 shrink-0 items-center justify-center overflow-hidden rounded-lg bg-[#FFCC00]">
        <Image src="/savills-logo.jpg" alt="Savills" width={44} height={44}/>
      </div>
      {!sidebarCollapsed&&<div>
        <div className="text-lg font-black tracking-tight">STL SAVILLS</div>
        <div className="text-[10px] uppercase tracking-[.2em] text-white/60">Technical Legal</div>
      </div>}
    </Link>
  ...
  <div className="p-4">
    {!sidebarCollapsed&&<div className="mb-4 rounded-xl bg-white/10 p-3">
    ...
    <nav className="space-y-1">
      {nav.filter(n=>n.href!=="/settings"||role==="ADMIN").map(n=>{
        const I=n.icon;
        ...
        return <Link
          ...
          className={`flex items-center gap-3 rounded-xl px-3 py-2.5 text-sm transition ${active?"bg-[#FFCC00] font-bold text-[#002A54]":"text-white/75 hover:bg-white/10 hover:text-white"} ${sidebarCollapsed?"justify-center":""}`}
        >
  ...
  <div className="absolute bottom-0 left-0 right-0 border-t border-white/10 p-4">
    <div className={`flex items-center gap-3 ${sidebarCollapsed?"justify-center":""}`}>
      <div className="flex h-9 w-9 shrink-0 items-center justify-center rounded-full bg-white/10 text-xs font-bold">SA</div>
      {!sidebarCollapsed&&<div className="min-w-0">
        <div className="truncate text-sm font-semibold">Usuario demo</div>
  ...
  <main className={`transition-all duration-300 ${sidebarCollapsed?"lg:pl-20":"lg:pl-72"}`}>
   <header className="sticky top-0 z-20 flex min-h-16 items-center gap-3 border-b border-slate-200 bg-white/95 px-4 py-2 backdrop-blur lg:px-8">
    <button className="rounded-xl p-2 hover:bg-slate-100" onClick={()=>{if(window.innerWidth>=1024){setSidebarCollapsed(v=>!v)}else{setOpen(true)}}} title="Mostrar u ocultar menú"><Menu className="h-5 w-5"/></button>
